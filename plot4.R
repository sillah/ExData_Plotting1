...
plot(strptime(paste(as.Date(powerData$Date,"%d/%m/%Y"),powerData$Time),
              "%Y-%m-%d %H:%M:%S"), powerData$Global_active_power,  type="l",
     lwd=1.5,xlab = "", ylab = "Global Active Power")

plot(strptime(paste(as.Date(powerData$Date,"%d/%m/%Y"),powerData$Time),
              "%Y-%m-%d %H:%M:%S"), powerData$Voltage,  type="l",
     lwd=1.5,xlab = "datetime",ylab = "Voltage")
...
plot(strptime(paste(as.Date(powerData$Date,"%d/%m/%Y"),powerData$Time),
              "%Y-%m-%d %H:%M:%S"), powerData$Global_reactive_power,  type="l",
     lwd=1.5,xlab = "datetime",ylab = "Global_reactive_power", ylim=c(0.0, 0.5))
